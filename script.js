document.addEventListener("DOMContentLoaded", function () {
  ...
  const PROJECTS = {
   "ces-postcard": {
    ...
    links: [
      { label: "Project", href: "https://cespostcard26.vercel.app/" },
    ]
  },
  ...
  "fw-2025": {
    ...
    links: [
      { label: "Project", href: "https://digital-museum-gallery.vercel.app/" },
    ]
  },
  ...
  "funko-pop": {
    ...
    links: [
      { label: "Video", href: "https://www.tiktok.com/@rug/video/7280624024344956206" },
    ]
  },
  ...
  "women-sports": {
    ...
    links: [
      { label: "Video", href: "https://drive.google.com/file/d/1w52GDZTHv2I71jhaPL5NBh8bRKQGw3lD/view" },
    ]
  },
};
  ...
  filterChips.forEach((chip) => {
    chip.addEventListener("click", function () {
      ...
      console.log("clicked:", chip.dataset.filter, "active?", chip.classList.contains("active"));
    });
  });
...
form.addEventListener("submit", async (e) => {
  ...
  // ✅ Modal wiring (unchanged, just organized)
  ...
  if (body) {
    ...
    const links = (project.links || [])
      .map(l => `<a class="inline-flex items-center px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-sm font-medium" href="${l.href}" target="_blank" rel="noopener">${l.label} ↗</a>`)
      .join("");

    body.innerHTML = `
      ${project.overview ? `<p class="text-slate-700 leading-relaxed">${project.overview}</p>` : ""}
      ...
      ${project.links?.length ? `
        <section>
          <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-2">Links</h3>
